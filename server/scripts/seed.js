...
async function seed() {
    ...
    for (const t of tables)
        (0, schema_1.execSql)(`DELETE FROM ${t}`);
    // Create company_settings table
    (0, schema_1.execSql)(`CREATE TABLE IF NOT EXISTS company_settings (
    key TEXT PRIMARY KEY,
    value TEXT NOT NULL
  )`);
    (0, schema_1.execSql)('DELETE FROM company_settings');
    // Company settings
    const settings = {
        company_name: 'RT',
    ...
    for (const [k, v] of Object.entries(settings)) {
        (0, schema_1.runSql)('INSERT INTO company_settings (key, value) VALUES (?, ?)', [k, v]);
    }
    // Users
    const adminHash = bcryptjs_1.default.hashSync('admin123', 10);
    const viewerHash = bcryptjs_1.default.hashSync('viewer123', 10);
    (0, schema_1.runSql)('INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)', ['admin', adminHash, 'admin']);
    (0, schema_1.runSql)('INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)', ['viewer', viewerHash, 'viewer']);
    // Projects
    const projectNames = [
    ...
    for (const p of projectNames)
        (0, schema_1.runSql)('INSERT INTO projects (name, description) VALUES (?, ?)', [p, `RT ${p} program`]);
    // Commodities
    ...
    for (const c of commodityNames)
        (0, schema_1.runSql)('INSERT INTO commodities (name) VALUES (?)', [c]);
    ...
    suppliers.forEach((s, i) => {
        (0, schema_1.runSql)(`INSERT INTO suppliers (supplier_id, company_name, country, city, street_address, latitude, longitude, default_incoterm, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [`SUP-${(i + 1).toString().padStart(4, '0')}`, s.company_name, s.country, s.city, s.address, s.lat, s.lng, s.incoterm, s.status,
    ...
    for (let i = 1; i <= suppliers.length; i++) {
        ...
        (0, schema_1.runSql)('INSERT INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [i, 'primary', null, `${firstNames[fi]} ${lastNames[li]}`, 'Sales Manager', `${firstNames[fi].toLowerCase()}.${lastNames[li].toLowerCase()}@${s.company_name.toLowerCase().replace(/[^a-z]/g, '').slice(0, 8)}.com`, `+${i > 30 ? '86' : '49'}-${100 + i}-${1000 + i}`]);
        (0, schema_1.runSql)('INSERT INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [i, 'secondary', null, `${firstNames[fn2]} ${lastNames[ln2]}`, 'Logistics Coordinator', `logistics@${s.company_name.toLowerCase().replace(/[^a-z]/g, '').slice(0, 8)}.com`, `+${i > 30 ? '86' : '49'}-${200 + i}-${2000 + i}`]);
        if (i % 3 === 0) {
            (0, schema_1.runSql)('INSERT INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [i, 'escalation', 1, `Director ${lastNames[(i + 1) % lastNames.length]}`, 'VP Supply Chain', `vp@${s.company_name.toLowerCase().replace(/[^a-z]/g, '').slice(0, 8)}.com`, `+${i > 30 ? '86' : '49'}-${300 + i}-${3000 + i}`]);
        }
    }
    // Assign projects and commodities
    for (let i = 1; i <= suppliers.length; i++) {
        (0, schema_1.runSql)('INSERT OR IGNORE INTO supplier_projects (supplier_id, project_id) VALUES (?, ?)', [i, ((i - 1) % 8) + 1]);
        if (i % 3 === 0)
            (0, schema_1.runSql)('INSERT OR IGNORE INTO supplier_projects (supplier_id, project_id) VALUES (?, ?)', [i, ((i) % 8) + 1]);
        if (i % 5 === 0)
            (0, schema_1.runSql)('INSERT OR IGNORE INTO supplier_projects (supplier_id, project_id) VALUES (?, ?)', [i, ((i + 3) % 8) + 1]);
        (0, schema_1.runSql)('INSERT OR IGNORE INTO supplier_commodities (supplier_id, commodity_id) VALUES (?, ?)', [i, ((i - 1) % 12) + 1]);
        if (i % 2 === 0)
            (0, schema_1.runSql)('INSERT OR IGNORE INTO supplier_commodities (supplier_id, commodity_id) VALUES (?, ?)', [i, ((i + 4) % 12) + 1]);
    }
    ...
    const routesData = [
        ...
        // Outbound routes
        {
            name: 'Zagreb-Munich Outbound', route_type: 'outbound', transport_mode: 'road', carrier: 'Kuehne+Nagel', days: 2,
        ...
        {
            name: 'Zagreb-Wolfsburg Outbound', route_type: 'outbound', transport_mode: 'road', carrier: 'DB Schenker', days: 3,
        ...
        {
            name: 'Zagreb-Torino Outbound', route_type: 'outbound', transport_mode: 'road', carrier: 'XPO Logistics', days: 2,
    ...
    for (const r of routesData) {
        const { lastId: routeId } = (0, schema_1.runSql)('INSERT INTO transport_routes (name, route_type, transport_mode, carrier_name, transit_days, waypoints) VALUES (?, ?, ?, ?, ?, ?)', [r.name, r.route_type, r.transport_mode, r.carrier, r.days, JSON.stringify(r.waypoints)]);
        for (const sid of r.supplier_ids) {
            (0, schema_1.runSql)('INSERT OR IGNORE INTO route_suppliers (route_id, supplier_id) VALUES (?, ?)', [routeId, sid]);
        }
    }
    (0, schema_1.saveDb)();
