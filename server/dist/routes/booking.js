...
function ensureBookingTables() {
    ...
    for (const m of newCols) {
        try {
            db.run(`SELECT ${m.col} FROM transport_requisitions LIMIT 1`);
        }
        catch {
            try {
                db.run(m.sql);
    ...
    try {
        ...
        const schemaSql = schemaRow[0]?.values[0]?.[0] || '';
        if (schemaSql && !schemaSql.includes('pending_approval')) {
            ...
            for (const [col, def] of migCols) {
                try {
                    db.run(`SELECT ${col} FROM transport_requisitions LIMIT 1`);
                }
                catch {
                    try {
                        db.run(`ALTER TABLE transport_requisitions ADD COLUMN ${col} ${def}`);
                    }
                    catch { }
                }
            }
...
router.get('/requisitions', auth_1.authenticate, (req, res) => {
    try {
        ensureBookingTables();
        const requisitions = (0, schema_1.queryAll)(`
    ...
    `);
        const enriched = requisitions.map((r) => {
            const quotes = (0, schema_1.queryAll)('SELECT * FROM forwarder_quotes WHERE requisition_id = ? ORDER BY price ASC', [r.id]);
...
router.get('/requisitions/:id', auth_1.authenticate, (req, res) => {
    try {
        ensureBookingTables();
        const requisition = (0, schema_1.queryOne)(`
      SELECT r.*, s.company_name as supplier_name, s.city as supplier_city
      FROM transport_requisitions r
      LEFT JOIN suppliers s ON s.id = r.supplier_id
      WHERE r.id = ?
    `, [req.params.id]);
        ...
        const quotes = (0, schema_1.queryAll)('SELECT * FROM forwarder_quotes WHERE requisition_id = ? ORDER BY price ASC', [requisition.id]);
...
function detectDeviations(plan, pickup_date, delivery_date, carrier, shipment_type, equipment) {
    const DAY_CODE_WEEKDAY = { M0: 1, T0: 2, W0: 3, R0: 4, F0: 5, S0: 6, Z0: 0, M1: 1, T1: 2, W1: 3, R1: 4, F1: 5, S1: 6 };
...
router.post('/requisitions', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        const { requestor_name, department, supplier_id, priority, direction, shipment_type, origin_id, origin_name, origin_zip, origin_city, origin_country, destination_id, destination_name, destination_zip, destination_city, destination_country, route_description, additional_origins, pickup_day_code, delivery_day_code, pickup_date, pickup_time, delivery_date, arrival_time, carrier, equipment, transit_days, customs, pallets, pallet_height, pallet_length, pallet_width, stackable, weight_kg, volume_m3, transport_mode, material_description, special_instructions, is_special_transport: clientSpecial, matched_route_plan_id, deviations: clientDeviations, deviation_justification, approval_status: clientApproval, volume_util_pct, weight_util_pct } = req.body;
        ...
        let serverDeviations = [];
        let isDeviation = false;
        let resolvedPlan = null;
        // Step 1: If we have a matched route plan, compare ALL fields server-side
        if (matched_route_plan_id) {
            resolvedPlan = (0, schema_1.queryOne)('SELECT * FROM route_plans WHERE id = ?', [matched_route_plan_id]);
            if (resolvedPlan) {
        ...
        if (!matched_route_plan_id && supplier_id && !clientSpecial) {
            const sup = (0, schema_1.queryOne)('SELECT supplier_id, company_name FROM suppliers WHERE id = ?', [supplier_id]);
            if (sup) {
                const dir = direction || 'inbound';
                const plans = (0, schema_1.queryAll)(`SELECT * FROM route_plans WHERE direction = ? AND (origin_id = ? OR origin_name = ? OR destination_id = ? OR destination_name = ?)`, [dir, sup.supplier_id, sup.company_name, sup.supplier_id, sup.company_name]);
                if (plans.length > 0) {
        ...
        const finalApproval = finalIsSpecial ? 'pending_approval' : 'approved';
        const finalPriority = finalIsSpecial ? 'urgent' : (priority || 'medium');
        // Generate req_number
        const lastReq = (0, schema_1.queryOne)('SELECT req_number FROM transport_requisitions ORDER BY id DESC LIMIT 1');
        let nextNum = 1001;
        ...
        const { lastId: id } = (0, schema_1.runSql)(`INSERT INTO transport_requisitions (
        ...
        const created = (0, schema_1.queryOne)(`
      SELECT r.*, s.company_name as supplier_name, s.city as supplier_city
      FROM transport_requisitions r
      LEFT JOIN suppliers s ON s.id = r.supplier_id
      WHERE r.id = ?
    `, [id]);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req_number, pickup_date, delivery_date || null,
        ...
        if (!finalIsSpecial && matched_route_plan_id && carrier) {
            try {
                (0, schema_1.execSql)(`UPDATE transport_requisitions SET assigned_forwarder = ?, status = 'assigned', updated_at = datetime('now') WHERE id = ?`, [carrier, id]);
                (0, schema_1.execSql)(`UPDATE shipments SET carrier_name = ?, status = 'assigned' WHERE shipment_number = ?`, [carrier, req_number]);
            }
            catch { /* best-effort */ }
        }
        // Re-fetch to get latest status
        const final = (0, schema_1.queryOne)(`
      SELECT r.*, s.company_name as supplier_name, s.city as supplier_city
...
router.put('/requisitions/:id/status', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        ...
        const current = (0, schema_1.queryOne)('SELECT status, is_special_transport, approval_status FROM transport_requisitions WHERE id = ?', [req.params.id]);
        if (!current)
            return res.status(404).json({ error: 'Requisition not found' });
        ...
        (0, schema_1.execSql)(`UPDATE transport_requisitions SET status = ?, updated_at = datetime('now') WHERE id = ?`, [status, req.params.id]);
        // Sync shipment record
        try {
            const req_data = (0, schema_1.queryOne)(`
        SELECT r.*, s.company_name as supplier_name, s.city as supplier_city
        FROM transport_requisitions r
        LEFT JOIN suppliers s ON s.id = r.supplier_id
        WHERE r.id = ?
      `, [req.params.id]);
            if (req_data) {
                const existingShipment = (0, schema_1.queryOne)('SELECT id FROM shipments WHERE shipment_number = ?', [req_data.req_number]);
                if (existingShipment) {
                    (0, schema_1.execSql)(`UPDATE shipments SET status = ?, carrier_name = COALESCE(?, carrier_name),
              total_cost_eur = CASE WHEN ? > 0 THEN ? ELSE total_cost_eur END,
              delivery_date = COALESCE(?, delivery_date),
              is_on_time = CASE WHEN ? = 'delivered' THEN 1 ELSE is_on_time END
            WHERE shipment_number = ?`, [status, req_data.assigned_forwarder,
                        req_data.assigned_price || 0, req_data.assigned_price || 0,
                        req_data.delivery_date,
                        status, req_data.req_number]);
                }
                else {
                    (0, schema_1.runSql)(`INSERT INTO shipments (shipment_number, shipment_date, delivery_date, supplier_id, supplier_name,
              origin_city, transport_mode, carrier_name, pallets_shipped, pallet_capacity,
              weight_kg, volume_m3, total_cost_eur, is_on_time, is_urgent, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req_data.req_number, req_data.pickup_date, req_data.delivery_date || null,
...
router.post('/requisitions/:id/quotes', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        ...
        (0, schema_1.runSql)(`INSERT INTO forwarder_quotes (requisition_id, forwarder_name, price, currency, transit_days, valid_until, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`, [req.params.id, forwarder_name, price, currency || 'EUR', transit_days || null, valid_until || null, notes || null]);
        // Auto-update status to quotes_received if still spot_requested
        const requisition = (0, schema_1.queryOne)('SELECT status FROM transport_requisitions WHERE id = ?', [req.params.id]);
        if (requisition?.status === 'spot_requested') {
            (0, schema_1.execSql)(`UPDATE transport_requisitions SET status = 'quotes_received', updated_at = datetime('now') WHERE id = ?`, [req.params.id]);
        }
...
router.put('/requisitions/:id/assign', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        ...
        (0, schema_1.execSql)(`UPDATE transport_requisitions SET assigned_forwarder = ?, assigned_price = ?, status = 'assigned', updated_at = datetime('now') WHERE id = ?`, [forwarder_name, price || null, req.params.id]);
        // Sync carrier and cost to the linked shipment record
        try {
            const req_data = (0, schema_1.queryOne)('SELECT req_number FROM transport_requisitions WHERE id = ?', [req.params.id]);
            if (req_data) {
                (0, schema_1.execSql)(`UPDATE shipments SET carrier_name = ?, total_cost_eur = ?, status = 'assigned' WHERE shipment_number = ?`, [forwarder_name, price || 0, req_data.req_number]);
            }
        }
        catch { /* best-effort */ }
...
// APPROVE/REJECT deviation transport (Operations Supervisor workflow)
router.put('/requisitions/:id/approve', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        const { action, reviewer_notes } = req.body; // action: 'approve' | 'reject'
        const requisition = (0, schema_1.queryOne)('SELECT * FROM transport_requisitions WHERE id = ?', [req.params.id]);
        if (!requisition)
            return res.status(404).json({ error: 'Requisition not found' });
        ...
        if (action === 'approve') {
            (0, schema_1.execSql)(`UPDATE transport_requisitions SET status = 'new', approval_status = 'approved', updated_at = datetime('now') WHERE id = ?`, [req.params.id]);
            res.json({ message: 'Deviation approved. Requisition moved to New for processing.' });
        }
        else if (action === 'reject') {
            (0, schema_1.execSql)(`UPDATE transport_requisitions SET status = 'cancelled', approval_status = 'rejected', updated_at = datetime('now') WHERE id = ?`, [req.params.id]);
            res.json({ message: 'Deviation rejected. Requisition cancelled.' });
        }
        else {
...
router.post('/spot-request', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        ensureBookingTables();
        ...
        for (const id of requisition_ids) {
            const req_data = (0, schema_1.queryOne)('SELECT id, status, is_special_transport, approval_status FROM transport_requisitions WHERE id = ?', [id]);
            if (!req_data)
                continue;
            ...
            (0, schema_1.execSql)(`UPDATE transport_requisitions SET status = 'spot_requested', updated_at = datetime('now') WHERE id = ?`, [id]);
            processed.push(id);
        }
