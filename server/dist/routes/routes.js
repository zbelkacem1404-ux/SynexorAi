...
router.get('/export/csv', auth_1.authenticate, (req, res) => {
    const routes = (0, schema_1.queryAll)('SELECT * FROM transport_routes ORDER BY created_at DESC');
    const rows = routes.map((r) => {
        const suppliers = (0, schema_1.queryAll)('SELECT s.supplier_id, s.company_name FROM suppliers s JOIN route_suppliers rs ON rs.supplier_id = s.id WHERE rs.route_id = ?', [r.id]);
        ...
        return {
            ...
            suppliers: suppliers.map((s) => s.supplier_id).join(';'),
            waypoints: waypoints.map((w) => `${w.lat},${w.lng}${w.label ? `,${w.label}` : ''}`).join(';')
        };
    });
...
router.post('/import/csv', auth_1.authenticate, auth_1.requireAdmin, upload.single('file'), (req, res) => {
    ...
    for (const record of records) {
        try {
            ...
            const waypoints = (waypointStr || '').split(';').filter(Boolean).map((wp) => {
            ...
            const { lastId: id } = (0, schema_1.runSql)('INSERT INTO transport_routes (name, route_type, transport_mode, carrier_name, transit_days, waypoints) VALUES (?, ?, ?, ?, ?, ?)', [name, route_type, transport_mode, carrier_name || null, transit_days ? parseInt(transit_days) : null, JSON.stringify(waypoints)]);
            // Link suppliers by supplier_id
            if (supplierStr) {
                const supplierIds = supplierStr.split(';').filter(Boolean);
                for (const sid of supplierIds) {
                    const supplier = (0, schema_1.queryOne)('SELECT id FROM suppliers WHERE supplier_id = ?', [sid.trim()]);
                    if (supplier) {
                        (0, schema_1.runSql)('INSERT OR IGNORE INTO route_suppliers (route_id, supplier_id) VALUES (?, ?)', [id, supplier.id]);
                    }
                }
            }
            imported++;
...
router.get('/export/json', auth_1.authenticate, (req, res) => {
    const routes = (0, schema_1.queryAll)('SELECT * FROM transport_routes ORDER BY created_at DESC');
...
router.get('/', auth_1.authenticate, (req, res) => {
    const routes = (0, schema_1.queryAll)('SELECT * FROM transport_routes ORDER BY created_at DESC');
    const enriched = routes.map((r) => {
        const suppliers = (0, schema_1.queryAll)('SELECT s.id, s.supplier_id, s.company_name, s.latitude, s.longitude FROM suppliers s JOIN route_suppliers rs ON rs.supplier_id = s.id WHERE rs.route_id = ?', [r.id]);
        // Get matching route plans by route name
        let routePlans = [];
        try {
            routePlans = (0, schema_1.queryAll)(`SELECT id, route_description, transport_mode, origin_name, destination_name, carrier, transit_time_days, direction, pickup_date, delivery_date FROM route_plans WHERE route_description LIKE ?`, [`%${r.name}%`]);
        }
...
router.get('/:id', auth_1.authenticate, (req, res) => {
    const route = (0, schema_1.queryOne)('SELECT * FROM transport_routes WHERE id = ?', [req.params.id]);
    if (!route)
        return res.status(404).json({ error: 'Route not found' });
    const suppliers = (0, schema_1.queryAll)('SELECT s.id, s.supplier_id, s.company_name FROM suppliers s JOIN route_suppliers rs ON rs.supplier_id = s.id WHERE rs.route_id = ?', [route.id]);
    res.json({ ...route, waypoints: JSON.parse(route.waypoints), suppliers });
});
// Helper: generate route_plan entries for a transport route
function generateRoutePlanEntries(routeId, body) {
    const { route_type, shipment_type, carrier_name, transit_days, supplier_ids, pickup_date, pickup_time, delivery_date, arrival_time, equipment, customs, route_plan_mode, tour_description, origin_id, origin_name, origin_zip, origin_city, origin_country, destination_id, destination_name, destination_zip, destination_city, destination_country } = body;
    ...
    if (!tourDesc && modeCode === 'MR' && supplier_ids?.length > 0) {
        const firstSup = (0, schema_1.queryOne)('SELECT * FROM suppliers WHERE id = ?', [supplier_ids[0]]);
        if (firstSup) {
    ...
    const existingDescs = (0, schema_1.queryAll)("SELECT route_description FROM route_plans WHERE transport_mode = ?", [modeCode])
        .map((r) => r.route_description);
    if (!supplier_ids || supplier_ids.length === 0) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [routeDesc, tourDesc, modeCode,
    ...
    for (let si = 0; si < supplier_ids.length; si++) {
        const sup = (0, schema_1.queryOne)('SELECT * FROM suppliers WHERE id = ?', [supplier_ids[si]]);
        if (!sup)
            continue;
...
function getNextSequence(existingDescs, prefix, offset = 0) {
...
router.post('/', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    const { name, route_type, transport_mode, shipment_type, carrier_name, transit_days, waypoints, supplier_ids, tour_description } = req.body;
    ...
    const { lastId: id } = (0, schema_1.runSql)(`INSERT INTO transport_routes (name, route_description, tour_description, route_type, transport_mode, shipment_type, carrier_name, transit_days, waypoints) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [name, name, tour_description || null, route_type, transport_mode, shipment_type || 'ftl', carrier_name || null, transit_days || null, JSON.stringify(waypoints)]);
    if (supplier_ids && Array.isArray(supplier_ids)) {
        for (const sid of supplier_ids)
            (0, schema_1.runSql)('INSERT OR IGNORE INTO route_suppliers (route_id, supplier_id) VALUES (?, ?)', [id, sid]);
    }
    ...
    const route = (0, schema_1.queryOne)('SELECT * FROM transport_routes WHERE id = ?', [id]);
    const suppliers = (0, schema_1.queryAll)('SELECT s.id, s.supplier_id, s.company_name, s.latitude, s.longitude FROM suppliers s JOIN route_suppliers rs ON rs.supplier_id = s.id WHERE rs.route_id = ?', [id]);
...
router.put('/:id', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    const existing = (0, schema_1.queryOne)('SELECT * FROM transport_routes WHERE id = ?', [req.params.id]);
    if (!existing)
        return res.status(404).json({ error: 'Route not found' });
    const { name, route_type, transport_mode, shipment_type, carrier_name, transit_days, waypoints, supplier_ids } = req.body;
    (0, schema_1.execSql)(`UPDATE transport_routes SET name=?, route_type=?, transport_mode=?, shipment_type=?, carrier_name=?, transit_days=?, waypoints=?, updated_at=datetime('now') WHERE id=?`, [name || existing.name, route_type || existing.route_type, transport_mode || existing.transport_mode,
    ...
    if (supplier_ids && Array.isArray(supplier_ids)) {
        (0, schema_1.execSql)('DELETE FROM route_suppliers WHERE route_id = ?', [req.params.id]);
        for (const sid of supplier_ids)
            (0, schema_1.runSql)('INSERT OR IGNORE INTO route_suppliers (route_id, supplier_id) VALUES (?, ?)', [req.params.id, sid]);
    }
    const updated = (0, schema_1.queryOne)('SELECT * FROM transport_routes WHERE id = ?', [req.params.id]);
...
router.delete('/:id', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ error: 'Route not found' });
    (0, schema_1.execSql)('DELETE FROM transport_routes WHERE id = ?', [req.params.id]);
