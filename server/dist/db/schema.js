...
const DB_PATH = path_1.default.join(__dirname, '..', '..', 'data', 'logistics.db');
let db = null;
...
async function initializeDb() {
    ...
    for (const m of migrationColumns) {
        try {
            d.run(`SELECT ${m.col} FROM transport_routes LIMIT 1`);
        }
        catch {
            try {
                d.run(m.sql);
...
function seedRelationships(d) {
    ...
    for (let i = 0; i < suppliers.length; i++) {
        ...
        d.run('INSERT OR IGNORE INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [sid, 'primary', null, `${firstNames[fi]} ${lastNames[li]}`, 'Sales Manager', `${firstNames[fi].toLowerCase()}.${lastNames[li].toLowerCase()}@${emailDomain}.com`, `+49-${100 + i}-${1000 + i}`]);
        d.run('INSERT OR IGNORE INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [sid, 'secondary', null, `${firstNames[fn2]} ${lastNames[ln2]}`, 'Logistics Coordinator', `logistics@${emailDomain}.com`, `+49-${200 + i}-${2000 + i}`]);
        if ((i + 1) % 3 === 0) {
            d.run('INSERT OR IGNORE INTO contacts (supplier_id, type, escalation_level, name, role_title, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)', [sid, 'escalation', 1, `Director ${lastNames[(i + 2) % lastNames.length]}`, 'VP Supply Chain', `vp@${emailDomain}.com`, `+49-${300 + i}-${3000 + i}`]);
        }
    }
    ...
    for (let i = 0; i < suppliers.length; i++) {
        ...
        if ((i + 1) % 3 === 0)
            d.run('INSERT OR IGNORE INTO supplier_projects (supplier_id, project_id) VALUES (?, ?)', [sid, ((i + 1) % 8) + 1]);
        if ((i + 1) % 5 === 0)
            d.run('INSERT OR IGNORE INTO supplier_projects (supplier_id, project_id) VALUES (?, ?)', [sid, ((i + 4) % 8) + 1]);
    }
    ...
    for (let i = 0; i < suppliers.length; i++) {
        ...
        if ((i + 1) % 2 === 0)
            d.run('INSERT OR IGNORE INTO supplier_commodities (supplier_id, commodity_id) VALUES (?, ?)', [sid, ((i + 5) % 12) + 1]);
    }
    // Seed route-supplier assignments (map supplier company names to IDs)
    const nameToId = {};
    for (const s of suppliers)
        nameToId[s.company_name] = s.id;
    ...
    while (routeStmt.step()) {
        const row = routeStmt.getAsObject();
        ...
        for (const [pattern, supplierNames] of Object.entries(routeSupplierMap)) {
            if (route.name.includes(pattern)) {
                for (const sn of supplierNames) {
                    ...
                    if (sid) {
                        try {
                            d.run('INSERT OR IGNORE INTO route_suppliers (route_id, supplier_id) VALUES (?, ?)', [route.id, sid]);
                        }
                        catch { /* skip */ }
                    }
                }
...
function seedRoutePlans(d) {
    ...
    const modeMap = { road: 'FTL', sea: 'FTL', air: 'FTL', rail: 'FTL', multimodal: 'HUB' };
    ...
    for (const route of allRoutes) {
        ...
        const routeSuppliers = [];
        while (supStmt.step())
            routeSuppliers.push(supStmt.getAsObject());
        supStmt.free();
        ...
        const customs = route.transport_mode === 'road' && ['AT', 'DE', 'SI', 'HU', 'SK', 'CZ', 'PL', 'RO'].some((c) => (route.name || '').includes(c)) ? '' : 'Yes';
        if (routeSuppliers.length === 0) {
            ...
            try {
                d.run(`INSERT INTO route_plans (route_description, tour_description, transport_mode, origin_id, origin_name, origin_zip, origin_city, origin_country,
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [routeDesc, '', mode, origin.id, origin.name, origin.zip || '', origin.city, origin.country,
            ...
            for (let si = 0; si < routeSuppliers.length; si++) {
              ...
              destination_id, destination_name, destination_zip, destination_city, destination_country,
              pickup_date, pickup_time, delivery_date, arrival_time, carrier, equipment, transit_time_days, customs, direction)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [routeDesc, route.name, mode, origin.id, origin.name, '', origin.city, origin.country,
