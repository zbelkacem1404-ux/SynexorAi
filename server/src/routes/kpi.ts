    ...
    for (const b of orphanBookings) {
      try {
        runSql(
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [b.req_number, b.pickup_date, b.delivery_date || null,
...
router.get('/compute', authenticate, (_req: Request, res: Response) => {
  ...
  const rawShipments = queryAll('SELECT * FROM shipments ORDER BY shipment_date DESC');

  // Also pull all non-cancelled bookings from transport_requisitions
  let bookingRecords: any[] = [];
  try {
    bookingRecords = queryAll(`
  ...
  const bookingAsShipments = bookingRecords.map((b: any) => ({
  ...
  const existingNumbers = new Set(rawShipments.map((s: any) => s.shipment_number));
  const uniqueBookings = bookingAsShipments.filter((b: any) => !existingNumbers.has(b.shipment_number));
  const shipments = [...rawShipments, ...uniqueBookings];
  const total = shipments.length;

  // --- Truck Utilization ---
  const fillRates = shipments.filter((s: any) => s.pallet_capacity > 0)
    .map((s: any) => (s.pallets_shipped / s.pallet_capacity) * 100);
  const avgFillRate = fillRates.length > 0 ? fillRates.reduce((a: number, b: number) => a + b, 0) / fillRates.length : 0;
  const emptyTruckCount = fillRates.filter((r: number) => r < 40).length;
  const emptyTruckRatio = (emptyTruckCount / Math.max(fillRates.length, 1)) * 100;
  const totalKm = shipments.reduce((s: number, r: any) => s + (r.total_km || r.distance_km || 0), 0);
  const emptyKm = shipments.reduce((s: number, r: any) => s + (r.empty_km || 0), 0);
  const deadheadRatio = totalKm > 0 ? (emptyKm / totalKm) * 100 : 0;
  // Weighted utilization: sum(pallets_shipped) / sum(pallet_capacity) * 100
  const totalPalletsShipped = shipments.reduce((s: number, r: any) => s + (r.pallets_shipped || 0), 0);
  const totalPalletCapacity = shipments.reduce((s: number, r: any) => s + (r.pallet_capacity || 33), 0);
  const avgUtilization = totalPalletCapacity > 0 ? (totalPalletsShipped / totalPalletCapacity) * 100 : 0;

  // --- Cost KPIs ---
  const totalCost = shipments.reduce((s: number, r: any) => s + (r.total_cost_eur || 0), 0);
  const totalWeight = shipments.reduce((s: number, r: any) => s + (r.weight_kg || 0), 0);
  const totalDistance = shipments.reduce((s: number, r: any) => s + (r.distance_km || 0), 0);
  ...
  const consolidatedCount = shipments.filter((s: any) => s.is_consolidated).length;
  ...
  const uniqueDays = new Set(shipments.map((s: any) => s.shipment_date?.substring(0, 10))).size;
  const transportFrequency = uniqueDays > 0 ? total / uniqueDays : 0;

  // --- Operational ---
  const onTimeCount = shipments.filter((s: any) => s.is_on_time).length;
  const onTimeDispatch = (onTimeCount / Math.max(total, 1)) * 100;
  const urgentCount = shipments.filter((s: any) => s.is_urgent).length;
  const urgentPct = (urgentCount / Math.max(total, 1)) * 100;

  // Mode mix
  const modeMap: Record<string, number> = {};
  shipments.forEach((s: any) => { modeMap[s.transport_mode] = (modeMap[s.transport_mode] || 0) + 1; });
  const modeMix = Object.entries(modeMap).map(([mode, count]) => ({
    mode, percentage: Math.round((count / total) * 100)
  })).sort((a, b) => b.percentage - a.percentage);

  // --- Advanced ---
  const totalMaterialValue = shipments.reduce((s: number, r: any) => s + (r.material_value_eur || 0), 0);
  ...
  const supplierMap: Record<string, { pallets: number; capacity: number; cost: number; count: number }> = {};
  shipments.forEach((s: any) => {
    const key = s.supplier_name || `Supplier #${s.supplier_id}`;
    if (!supplierMap[key]) supplierMap[key] = { pallets: 0, capacity: 0, cost: 0, count: 0 };
  ...
  // Monthly trend
  const monthMap: Record<string, { pallets: number; capacity: number }> = {};
  shipments.forEach((s: any) => {
    const m = (s.shipment_date || '').substring(0, 7); // YYYY-MM
    if (!monthMap[m]) monthMap[m] = { pallets: 0, capacity: 0 };
  ...
  // Route plan stats
  let routePlanStats: any = null;
  try {
    routePlanStats = queryOne(`SELECT
...
router.get('/shipments', authenticate, (req: Request, res: Response) => {
  const limit = parseInt(req.query.limit as string) || 500;
  const offset = parseInt(req.query.offset as string) || 0;
  const shipments = queryAll('SELECT * FROM shipments ORDER BY shipment_date DESC LIMIT ? OFFSET ?', [limit, offset]);
  const countRow = queryOne('SELECT COUNT(*) as cnt FROM shipments');
  res.json({ shipments, total: countRow?.cnt || 0 });
});

// POST /kpi/shipments — create single shipment
router.post('/shipments', authenticate, requireAdmin, (req: Request, res: Response) => {
  const s = req.body;
  ...
  const { lastId } = runSql(
    ...
    VALUES (?,?,?,?,?, ?,?,?,?,?,?,?, ?,?,?,?,?,?,?, ?,?,?,?,?,?,?)`,
    [num, s.shipment_date, s.delivery_date || null, s.supplier_id || null, s.supplier_name || null,
  ...
  const created = queryOne('SELECT * FROM shipments WHERE id = ?', [lastId]);
  res.status(201).json(created);
});

// POST /kpi/shipments/import — bulk import from CSV
router.post('/shipments/import', authenticate, requireAdmin, upload.single('file'), (req: Request, res: Response) => {
  ...
  for (const row of records) {
    try {
      ...
      if (!shipDate) { errors.push(`Row ${imported + errors.length + 2}: missing date`); continue; }
      ...
      if (!supplierId && supplierName) {
        const found = queryOne('SELECT id FROM suppliers WHERE company_name = ? COLLATE NOCASE', [supplierName.trim()]);
...
router.get('/shipments/export', authenticate, (req: Request, res: Response) => {
  const shipments = queryAll('SELECT * FROM shipments ORDER BY shipment_date DESC');
...
router.delete('/shipments', authenticate, requireAdmin, (_req: Request, res: Response) => {
  execSql('DELETE FROM shipments');
